...
def _send_single_message(client, sender, recipient, message_part, index, total_parts):
    try:
...
def _split_message(body, max_length):
    message_parts = []
    ...
    while start_index < body_length:
        ...
        end_index = _find_last_boundary(body, start_index, end_index)
...
def _find_last_boundary(body, start_index, end_index):
...
def send_message(sender, recipient, body):
    ...
    message_parts = _split_message(body, MAX_MESSAGE_LENGTH)
    ...
    for i, message_part in enumerate(message_parts):
        ...
        _send_single_message(client, sender, recipient, message_part, i, len(message_parts))


def create_twiml_response(body=None):
